...
- Keep “Schritt X von Y” and the first instruction segment as one literal text node in one visually hidden semantic heading. Expose the separate visible step label, first visible segment, and visual progress indicator as presentational only so VoiceOver cannot split or repeat the combined unit. Later segments remain separate reading units. After the instruction paragraphs, place “Nächster Schritt” before “Vorheriger Schritt” in DOM and focus order.
